...
SNAPSHOT_PATH="${SNAPSHOT_PATH:-/home/avi/projects/orbital/clients/stake-link/sdl/orchestration/intelligence/data/cre_laa_snapshot.json}"
...
def parse_output(text: str) -> dict | None:
    ...
    # Strategy 1: LAA_CRE_OUTPUT_JSON= marker
    marker_matches = re.findall(r"LAA_CRE_OUTPUT_JSON=(\{.*\})", text)
    if marker_matches:
        try:
    ...
    required = {"signal", "poolState", "premiumQuotes"}
...
if not parsed:
    print("ERROR: Could not find LAA_CRE_OUTPUT_JSON marker in CRE output.", file=sys.stderr)
    raise SystemExit(1)
...
print(f"Wrote LAA snapshot to {snapshot_path}")
PY
