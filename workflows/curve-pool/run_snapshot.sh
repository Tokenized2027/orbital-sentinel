...
SNAPSHOT_PATH="${SNAPSHOT_PATH:-${ROOT_DIR}/../../intelligence/data/sentinel_curve_pool_snapshot.json}"
...
def parse_output(text: str) -> dict | None:
    ...
    # Strategy 1: CURVE_POOL_OUTPUT_JSON= marker
    marker_matches = re.findall(r"CURVE_POOL_OUTPUT_JSON=(\{.*\})", text)
    if marker_matches:
        try:
    ...
    required = {"timestamp", "chainName", "pool"}
...
if not parsed:
    print("ERROR: Could not find CURVE_POOL_OUTPUT_JSON marker in CRE output.", file=sys.stderr)
    raise SystemExit(1)

snapshot = {
    "generated_at_utc": datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
    "chain_name": parsed.get("chainName", "unknown"),
    "source": "cre_simulate",
    **parsed,
}
...
print(f"Wrote curve pool snapshot to {snapshot_path}")
PY
